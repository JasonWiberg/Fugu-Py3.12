...
class NEAT_FF(Brick):
    ...
    def _instantiate_genome(self, genome, graph, input_list, control_nodes, genome_name=None):
        input_list = np.asarray(input_list).flatten()
    
        if self.buffer:
            new_input_list = []
            for idx, input_neuron in enumerate(input_list):
                name = self.get_genome_neuron_number(genome_name, 'buffer_{}'.format(input_neuron))
                graph.add_node(name, threshold=0.5, decay=1.0, p=1.0)
                graph.add_edge(input_neuron, name, weight=1.0, delay=1)
        ...
        def neuron_name_from_number(neuron_no):
            if neuron_no >= 0:
                return self.get_genome_neuron_number(genome_name, neuron_no)
            else:
                return input_list[-neuron_no-1]
        ...
        for layer_no, layer in enumerate(layers):
            for neuron in layer:
                # FIX: Ensure neuron is treated as a scalar if it somehow comes as a set
                if isinstance(neuron, set):
                    for n in neuron:
                        neuron_layers[n] = layer_no + 1
                else:
        ...
        for neuron_no in genome.nodes:
            ...
            graph.add_node(neuron_name, threshold=threshold, decay=1.0, index=(neuron_no,), p=1.0)
        ...
        for source, target in genome.connections:
            ...
            if source in neuron_layers and target in neuron_layers:
                delay = max(1, neuron_layers[target]-neuron_layers[source])
            else:
                delay = 1
